    ...
    def __init__(self, args, training=True):
        if args.run_modelarts:
            ...
            if args.fold_train:
                ...
                val_dir = os.path.join(args.data_url,'fold'+str(args.fold), "val")
            else :
                train_dir = os.path.join(args.data_url, 'train') #change for food
                val_dir = os.path.join(args.data_url, "test") #验证用            
            # train_dir = os.path.join(args.data_url, 'fold0','train') #change for food
            # val_dir = os.path.join(args.data_url,'fold0', "val")
            if training:
                self.train_dataset = create_dataset_imagenet(train_dir, training=True, args=args)                
                self.val_dataset = create_dataset_imagenet(val_dir, training=False, args=args) #暂时没有划分val和train，现在凑合用
...
def create_dataset_imagenet(dataset_dir, args, repeat_num=1, training=True):
    ...
    if device_num == 1 or not training:
        if args.eval == 0:
            data_set = ds.ImageFolderDataset(dataset_dir, num_parallel_workers=args.num_parallel_workers,  #val_data_set
                                         shuffle=shuffle)
        else:
        ...
        data_set = ds.ImageFolderDataset(dataset_dir, num_parallel_workers=args.num_parallel_workers, shuffle=shuffle,    # train_data_set
